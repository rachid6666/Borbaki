...
export const ChooseForYou = async () => {
    try {
      
      const hotel = await hotel.aggregate([{ $sample: { size: 1 } }]);
  
      
      const rooms = await room.find({ hotelId: hotel[0]._id });
  
      
      const room = rooms[Math.floor(Math.random() * rooms.length)];
  
      return { hotel: hotel[0], room };
    } catch (err) {
      console.error(err);
    }
  };
